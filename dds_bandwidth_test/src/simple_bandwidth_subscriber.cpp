...
class SimpleBandwidthSubscriber : public rclcpp::Node
{
public:
  SimpleBandwidthSubscriber() : Node("simple_bandwidth_subscriber")
  {
    // Parameters
    ...
    // Create subscriber for string messages only
    string_sub_ = this->create_subscription<std_msgs::msg::String>(
      topic_name_, 10,
      std::bind(&SimpleBandwidthSubscriber::string_callback, this, std::placeholders::_1));

    // Create timer for periodic reporting
    report_timer_ = this->create_wall_timer(
      std::chrono::duration<double>(report_interval_),
      std::bind(&SimpleBandwidthSubscriber::report_statistics, this));
    ...
    RCLCPP_INFO(this->get_logger(), 
      "Starting simple bandwidth subscriber on topic: %s", topic_name_.c_str());
  }
...
int main(int argc, char** argv)
{
  ...
  auto node = std::make_shared<SimpleBandwidthSubscriber>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
} 